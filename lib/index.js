...
const validator = (api, { path, operation, validated, parameters = true, responses = true } = {}) => {
    ...
    return Co(function* () {
        ...
        if (path) {
            ...
            if (parameters === true) {
                buildParamValidators(pathObj, operationObj, result);
            }
            //Build validators for responses.
            if (responses === true && operationObj.responses) {
                buildRespValidators(operationObj, result);
            }
        } else {
            /**
             * All paths
            ...
            Object.keys(paths).forEach(path => {
                ...
                Object.keys(pathObj).forEach(operation => {
                    ...
                    if (OPERATIONS.includes(operation)) {
                        ...
                        //Build validators for parameters
                        if (parameters === true) {
                            buildParamValidators(pathObj, opsObj, result[path][operation]);
                        }
                        //Build validators for responses.
                        if (responses === true && opsObj.responses) {
                            buildRespValidators(opsObj, result[path][operation]);
                        }
                    }
                });
            });
